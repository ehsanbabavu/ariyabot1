...
export default function UserDashboard() {
  ...
  // Get unpaid and pending orders list for dashboard (for level 1 users)
  const { data: unpaidPendingOrders = [] } = useQuery<OrderWithDetails[]>({
    ...
    select: (data) => data.filter(order => order.status === 'awaiting_payment' || order.status === 'pending'),
  });
      ...
      <div className="space-y-4" data-testid="dashboard-content">
        ...
        {user?.role !== "user_level_2" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            {/* Unpaid and Pending Orders Card - Only for user_level_1 */}
              ...
              <Card className="text-center hover:shadow-md transition-shadow border-orange-200 bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-950/30 dark:to-orange-900/20">
                <CardContent className="p-3">
                  <div className="flex items-center justify-center mb-1">
                    ...
                    <span className="text-xs text-orange-700 dark:text-orange-300 font-medium">سفارشات در انتظار پرداخت و در انتظار تایید</span>
                  </div>
                  <div className="text-xl font-bold text-orange-600 mb-1" data-testid="stat-pending-payment-and-approval-orders">
        ...
        {/* Unpaid and Pending Orders List - Only for user_level_1 */}
        {user?.role === "user_level_1" && unpaidPendingOrders.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <AlertCircle className="h-5 w-5 text-orange-500" />
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                سفارشات در انتظار پرداخت و در انتظار تایید
              </h2>
            </div>
                  ...
                  <div 
                    ...
                    className={`p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${
                      order.status === 'awaiting_payment' 
                        ? 'border-r-4 border-yellow-400 bg-yellow-50/30 dark:bg-yellow-900/10' 
                        : 'border-r-4 border-orange-400 bg-orange-50/30 dark:bg-orange-900/10'
                    }`}
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                              ...
                              {order.status === 'awaiting_payment' ? 'در انتظار پرداخت' : 'در انتظار تایید'}
                            </div>
                          </div>
